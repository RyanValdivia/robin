...
export function MemoryPanel() {
  ...
  return (
    <div className="h-full flex flex-col md:flex-row overflow-hidden">
      <aside className="w-full md:w-72 max-h-40 md:max-h-none border-b md:border-b-0 md:border-r border-border overflow-y-auto shrink-0">
        {groups.map((group) => (
          <div key={group.type} className="px-4 pt-4">
            <div className="text-xs uppercase tracking-wide text-muted mb-1.5">{group.type}</div>
            {group.notes.map((note) => (
              <button
                key={note.path}
                onClick={() => openNote(note)}
                className={cn(
                  "block w-full text-left px-2.5 py-2 rounded-lg mb-1 text-sm",
                  selected?.path === note.path ? "bg-panel2 text-white" : "text-gray-300 hover:bg-panel2/60",
                )}
              >
      ...
      <div className="flex-1 overflow-y-auto px-4 sm:px-8 py-6">
        ...
        {selected && (
          <div className="max-w-2xl">
            ...
            <h2 className="text-lg font-semibold text-white mb-4">{selected.name || selected.path}</h2>
